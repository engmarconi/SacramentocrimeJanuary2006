...
function buildMarkersPoints(markersArray) {
    let parentDiv = document.getElementById("Markers");
    ...
    for (let i = 0; i < childrenEl.length;  i++) {
        ...
        let message = childrenEl[i].getAttribute("message");

        markersArray.push({ lat: lat, lng: lng, title: message });
    }
...
function buldOkaforPoint(pointsArray) {
    ...
    for (let i = 0; i < childrenEl.length;  i++) {
        ...
        pointsArray.push({ lat: lat, lng: lng, title: message });
    }
}
...
function initMap() {
    ...
    var mapOptions = {
        center: new google.maps.LatLng(okafor[0].lat, okafor[0].lng),
        zoom: 8,
        maxZoom: 18,
